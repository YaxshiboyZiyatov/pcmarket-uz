...
import com.example.pcmarketuz.service.OutputProductService;
...
//        return ResponseEntity.ok(all);
    ...
    @PostMapping
    public HttpEntity<?> added(@Valid @RequestBody OutputProductDto outputProductDto){
        ApiResponse add = outputProductService.add(outputProductDto);
        return ResponseEntity.status(add.isSuccess()? HttpStatus.CREATED:HttpStatus.CONFLICT).body(add);
    }
    @PutMapping("/{id}")
    public HttpEntity<?> edit(@Valid @PathVariable Integer id, @RequestBody OutputProductDto outputProductDto){
